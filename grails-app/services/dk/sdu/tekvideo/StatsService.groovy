...
class StatsService {
    ...
    private ServiceResult checkAccess(Node node) {
        if (!courseManagementService.canAccessNode(node)) {
            return fail(message: "Ikke tilladt", suggestedHttpStatus: HttpStatus.UNAUTHORIZED.value())
        }
        return ok()
    }
    ...
    private ProgressTable progressTableFromExercises(Course course, List<Exercise> leaves) {
        ...
        Map<ExerciseProgress, WrittenGroupStreak> writtenStreaks =
                progressReports.isEmpty() ? Collections.emptyMap() :
                        WrittenGroupStreak.findAllByProgressInList(progressReports)
                                .groupBy { it.progress }
                                .collectEntries {
                            [(it.key): it.value.first()]
                        } as Map<ExerciseProgress, WrittenGroupStreak>

        Map<ExerciseProgress, Integer> visits =
                progressReports.isEmpty() ? Collections.emptyMap() :
                        ExerciseVisit.findAllByProgressInList(progressReports)
                                .groupBy { it.progress }
    ...
    private ViewingStatistics views(List<Exercise> leaves, ViewingStatisticsConfiguration config) {
        ...
        def views = progress.size() > 0 ? ExerciseVisit.findAllByProgressInList(progress) : []
